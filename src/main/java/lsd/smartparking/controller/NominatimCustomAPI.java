...
public class NominatimCustomAPI {
    ...
    public static NominatimCustomAPI with(String endpointUrl) {
    	NominatimCustomAPI nominatimAPI = new NominatimCustomAPI();
    ...
    private OpenStreetMapResponse getLocationFrom(MapPoint mapPoint, Address address) {
        ...
        Map<String, String> responsePair = null;
        String firstKey = "";
        try {
            responsePair = getOpenStreetMap(mapPoint, address, mapPoint != null ? URL_SUFFIX_FOR_REVERSE_GECODING : URL_SUFFIX_FOR_GECODING);
        	firstKey = responsePair.keySet().toArray()[0].toString();
            String responseStr = responsePair.get(firstKey);
        ...
        if (responsePair != null && firstKey != null && openStreetMapResponse != null) {
            openStreetMapResponse.setUrl(firstKey);

        }
        return openStreetMapResponse;
    }

    private Map<String, String> getOpenStreetMap(MapPoint mapPoint, Address address, String urlSuffix) {
        ...
        map.put(urlWithLatLon, responseStr);
        return map;
    }
